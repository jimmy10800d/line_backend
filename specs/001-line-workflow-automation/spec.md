...
使用者可以透過 LINE 對話或網頁介面建立自訂的工作流程，定義觸發條件、執行步驟和輸出動作，系統將儲存並可重複執行這些流程。
...
- 當外部服務 OAuth Token 過期時，系統如何通知使用者重新授權？
- 當工作流程步驟執行到一半失敗時，如何處理已執行的步驟？（記錄狀態，支援斷點續傳）
- 當使用者輸入格式錯誤的指令時，系統如何提供有用的錯誤提示？
...
- **FR-002**: 系統必須支援指令解析（支援 /command 格式和自然語言）
...
- **FR-011**: 系統必須支援至少 3 種外部服務整合 [NEEDS CLARIFICATION: 請確認優先整合哪些服務？建議：Google Calendar、Notion、GitHub]
...
- 系統部署在具有穩定網路連線的伺服器環境
- 外部服務（Google、Notion 等）提供穩定的 API 服務
- 使用者願意授權存取其外部服務帳號
